    ...
    <main aria-label="Spatial Vision" className="max-w-7xl mx-auto px-4 py-6 space-y-6 flex flex-col h-[calc(100vh-6rem)]">
      ...
      <div className={`relative flex-1 rounded-3xl overflow-hidden bg-black border-2 transition-colors duration-300 shadow-2xl ${
        ...
        <div className="absolute bottom-6 left-4 right-4 sm:left-6 sm:right-6">
          <div className="bg-white/95 backdrop-blur-xl border border-slate-200 rounded-2xl p-4 flex items-center space-x-4 shadow-2xl">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center border shrink-0 ${
              isProcessing ? 'bg-accent/20 border-accent/50 animate-pulse' : 'bg-slate-100 border-slate-200'
            }`}>
        ...
        {/* Search Overlay */}
        <div className="absolute top-4 left-4 right-4 sm:top-6 sm:left-auto sm:right-6 sm:w-72">
          <form onSubmit={handleManualSearch} className="flex flex-col gap-2 bg-white/95 backdrop-blur-xl p-3 rounded-2xl border border-slate-200 shadow-xl">
            <div className="flex items-center space-x-2">
              <button
                ...
                className={`p-3 rounded-xl transition-colors shrink-0 ${
                  voiceState === 'listening' ? 'bg-danger text-white animate-pulse' : 'bg-slate-100 hover:bg-slate-200 text-accent'
                }`}
              >
            ...
            {targetQuery && (
               <div className="px-2 py-1.5 rounded-lg bg-accent/10 border border-accent/20 flex items-center justify-between">
                 <span className="text-[10px] font-mono text-accent font-bold">TARGET: {targetQuery.toUpperCase()}</span>
        ...
        {/* Mobile Tap-to-Start Voice Overlay */}
        {voiceState === 'idle' && (
          <div 
            className="absolute inset-0 z-10 flex items-center justify-center bg-black/40 backdrop-blur-sm cursor-pointer"
            onClick={() => {
              startListening();
